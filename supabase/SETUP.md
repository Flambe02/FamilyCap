...
- `migrations/20260823_family_video_notify_all.sql` (**après** `20260821_family_video_publish_schedule.sql`) : ajoute `notify_all` sur `family_videos`, qui découple le public du pop-up de connexion (destinataires sélectionnés, ou tout le monde) de `visibility_scope` (accès à la bibliothèque Souvenirs — mêmes destinataires, ou toute la famille) : les deux réglages sont désormais indépendants sur le formulaire « Publier un message vidéo ». Sans elle, le sous-onglet Administration « Messages vidéo » renvoie une erreur claire (409).
- `migrations/20260824_family_video_recipient_kind.sql` (**après** `20260724_family_videos.sql`) : ajoute `is_notify` / `is_library` sur `family_video_recipients` (défaut `true` sur les deux, sans perte de données), et met à jour `public.can_view_video()` en conséquence. Sépare la liste de destinataires en DEUX listes réellement indépendantes sur le formulaire « Publier un message vidéo » — un membre peut être destinataire du pop-up sans l'être de la bibliothèque Souvenirs, ou l'inverse, sur la même vidéo. Sans elle, le sous-onglet Administration « Messages vidéo » renvoie une erreur claire (409).
