...
export async function GET(request: Request) {
  try {
    ...
    const rows = await supabaseRest<Array<{ id: string; asset_id: string; ex_date: string; payment_date: string | null; amount_per_share: number | null; currency: string | null; status: string }>>(`corporate_actions?select=id,asset_id,ex_date,payment_date,amount_per_share,currency,status&action_type=eq.dividend&asset_id=in.(${assets.map((asset) => asset.id).join(",")})&order=ex_date.desc`);
