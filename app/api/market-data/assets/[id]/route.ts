...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const yahooSymbol = String(body.yahooSymbol ?? "").trim().toUpperCase() || null;
    const dataProvider = providerSymbol ? "eodhd" : yahooSymbol ? "yahoo" : "manual";
    await supabaseRest(`holdings?id=eq.${encodeURIComponent(id)}`, {
      method: "PATCH", headers: { prefer: "return=minimal" },
      body: JSON.stringify({ asset_type: assetType, symbol: String(body.ticker ?? "").trim().toUpperCase() || null, isin: String(body.isin ?? "").trim().toUpperCase() || null, provider_symbol: providerSymbol, yahoo_symbol: yahooSymbol, exchange: String(body.exchange ?? "").trim() || null, mic_code: String(body.micCode ?? "").trim().toUpperCase() || null, currency, country: String(body.country ?? "").trim() || null, data_provider: dataProvider, quote_mode: QUOTE_MODES.has(String(body.quoteMode ?? "")) ? body.quoteMode : providerSymbol ? "eod" : yahooSymbol ? "delayed" : "manual", classification_status: "verified", updated_at: new Date().toISOString() }),
    });
