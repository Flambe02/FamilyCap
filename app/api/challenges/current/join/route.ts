...
// Inscription du membre au défi courant + GEL de son objectif (target_amount_snapshot). Le
// member_id est FORCÉ depuis la session : un membre ne peut jamais s'inscrire pour un autre.
// Le statut « completed » et les points ne sont JAMAIS acceptés du client (calculés serveur).
export const runtime = "nodejs";

export async function POST(request: Request) {
  try {
    const viewer = await requireFamilyMember(request);
    const result = await joinChallenge(viewer.id);
    if (!result.ok) {
