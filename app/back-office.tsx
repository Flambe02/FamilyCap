...
function GiftEntryModal({ onClose, onSave }: { onClose: () => void; onSave: (record: GiftRecord) => void }) {
  ...
  return <div className="modal-backdrop" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><section className="modal gift-modal" role="dialog" aria-modal="true" aria-labelledby="gift-title"><header><div><span>CADEAU BTC · ÉTAPE {step} SUR 3</span><h2 id="gift-title">{step === 1 ? "Enregistrer l’achat" : step === 2 ? "Où sont les BTC ?" : "Contrôler et valider"}</h2></div><button onClick={onClose} aria-label="Fermer">×</button></header><div className="step-progress"><span className={step >= 1 ? "done" : ""} /><span className={step >= 2 ? "done" : ""} /><span className={step >= 3 ? "done" : ""} /></div><form onSubmit={submit}>
