...
type FamilyUser = { id: string; name: string; email?: string | null; role: DbRole; birthday_day?: number | null; birthday_month?: number | null; birthday_year?: number | null; access_status: string; is_active?: boolean; auth_user_id?: string | null; investment_access_scope?: "family" | "selected"; selected_viewer_ids?: string[] };
type Draft = { name: string; email: string; role: EditableRole; birthday: string; accessScope: "family" | "selected"; selectedViewerIds: string[] };

const EMPTY_DRAFT: Draft = { name: "", email: "", role: "adult", birthday: "", accessScope: "family", selectedViewerIds: [] };
...
function userDraft(user: FamilyUser): Draft { return { name: user.name, email: user.email ?? "", role: user.role === "child" ? "adult" : user.role, birthday: birthdayValue(user), accessScope: user.investment_access_scope ?? "family", selectedViewerIds: user.selected_viewer_ids ?? [] }; }
...
export function AdminUsers() {
  ...
  async function saveUser(event: FormEvent<HTMLFormElement>, user: FamilyUser) {
    ...
    try {
      await request("/api/admin/users", { method: "PATCH", body: JSON.stringify({ id: user.id, name: draft.name, email: draft.email, role: draft.role, birthday: draft.birthday, accessScope: draft.accessScope, selectedViewerIds: draft.selectedViewerIds }) }, "Modification impossible");
