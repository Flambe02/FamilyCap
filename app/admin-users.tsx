...
export function AdminUsers() {
  ...
  async function request(path: string, init: RequestInit, fallback: string): Promise<{ error?: string; message?: string; invitation?: { sent?: boolean; reason?: string } }> {
  ...
  async function createUser(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const result = await request("/api/admin/users", { method: "POST", body: JSON.stringify({ name: draft.name, email: draft.email, role: draft.role, birthday: draft.birthday, sendInvite: true, redirectTo: `${window.location.origin}/?auth=invite` }) }, "Invitation impossible");
      if (!result.invitation?.sent) throw new Error(result.invitation?.reason ?? "Le membre a été enregistré, mais l’invitation n’a pas été envoyée.");
  ...
  async function runAction(user: FamilyUser, action: "invite" | "reset_password" | "toggle" | "delete") {
    ...
    try {
      ...
      else { const result = await request("/api/admin/users/actions", { method: "POST", body: JSON.stringify({ action, memberId: user.id, redirectTo: `${window.location.origin}/?auth=${action === "invite" ? "invite" : "recovery"}` }) }, "Envoi impossible"); setMessage(result.message ?? "E-mail envoy\u00e9."); }
