...
function Accounts(){
  ...
  const [a,setA]=useState({memberId:"",name:"",accountType:"pea",institution:"",currency:"EUR",accountNumberLast4:"",ibanLast4:"",walletAddress:"",network:"bitcoin-mainnet",notes:"",openedAt:"",monthlyTarget:""});
  ...
  useEffect(()=>{const timer=window.setTimeout(()=>{void load().catch(e=>setMessage(e.message));},0);return()=>window.clearTimeout(timer);},[load]);
  const isInvestment=(t:string)=>t==="pea"||t==="securities";
  async function addAccount(e:FormEvent){e.preventDefault();try{const payload:Record<string,unknown>={memberId:a.memberId,name:a.name,accountType:a.accountType,institution:a.institution,currency:a.currency,accountNumberLast4:a.accountNumberLast4,ibanLast4:a.ibanLast4,walletAddress:a.walletAddress,network:a.network,notes:a.notes};if(isInvestment(a.accountType)){if(a.openedAt)payload.openedAt=a.openedAt;if(a.monthlyTarget.trim()!=="")payload.monthlyTarget=Number(a.monthlyTarget.replace(",","."));}await api("/api/admin/accounts",{method:"POST",body:JSON.stringify(payload)});setMessage("Compte ajouté.");setAccountOpen(false);await load();}catch(x){setMessage(x instanceof Error?x.message:"Ajout impossible");}}
  async function archive(account:Account){const active=account.is_active!==false;try{await api("/api/admin/accounts",{method:"PATCH",body:JSON.stringify({id:account.id,isActive:!active})});setMessage(active?"Compte archivé (historique conservé).":"Compte réactivé.");await load();}catch(x){setMessage(x instanceof Error?x.message:"Modification impossible");}}
  ...
  async function remove(kind:"accounts"|"holdings",id:string,label:string){if(!window.confirm("Supprimer "+label+" ?"))return;try{const response=await fetch("/api/admin/"+kind+"?id="+encodeURIComponent(id),{method:"DELETE",headers:await headers()});const result=await response.json().catch(()=>({}));if(response.status===409&&result.requiresConfirmation){if(!window.confirm(result.error+"\n\nConfirmer la suppression DÉFINITIVE (opérations comprises) ? Préférez l'archivage pour conserver l'historique."))return;const forced=await fetch("/api/admin/"+kind+"?id="+encodeURIComponent(id)+"&force=true",{method:"DELETE",headers:await headers()});if(!forced.ok)throw new Error((await forced.json().catch(()=>({}))).error||"Suppression impossible");}else if(!response.ok){throw new Error(result.error||"Suppression impossible");}setMessage("Élément supprimé.");await load();}catch(x){setMessage(x instanceof Error?x.message:"Suppression impossible");}}
