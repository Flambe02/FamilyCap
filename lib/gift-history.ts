...
const birthdays: HistoricalGift[] = [
  ...
  { member: "Uhaina", occasion: "Anniversaire", giftDate: "2023-08-16", purchaseDate: "2023-08-16", amountEur: 55, btcAmount: 0.00102021, note: "Valeur confirmée par le tableau familial." },
...
const christmasByYear: Array<{ date: string; amounts: Record<HistoricalGift["member"], number> }> = [
  { date: "2022-12-27", amounts: { Thibault: 0.003094, Uhaina: 0.003094, Paul: 0.003094, Aurore: 0.003094, Thomas: 0.002894 } },
...
const christmas = christmasByYear.flatMap(({ date, amounts }) => Object.entries(amounts).map(([member, btcAmount]) => ({
  ...
  amountEur: 55,
  btcAmount,
  note: `Cadeau de Noël acheté le ${date.split("-").reverse().join("/")} · valeur confirmée par le tableau familial.`,
})));
